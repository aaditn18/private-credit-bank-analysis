...
OUTPUT_PNG = Path(__file__).resolve().parent / "Private_Credit_Hybrid_BarChart.png"
...
def generate_hybrid_sentiment_chart():
    ...
    print("Step 2 & 3: Extracting Call Report Data and Entity Resolution...")
    ...
    df_quant_filtered['RCFD1563'] = pd.to_numeric(df_quant_filtered['RCFD1563'], errors='coerce')
    df_quant_filtered['RCON1545'] = pd.to_numeric(df_quant_filtered['RCON1545'], errors='coerce')
    df_quant_filtered['Private_Credit_Proxy'] = df_quant_filtered['RCFD1563'].fillna(df_quant_filtered['RCON1545'])

    print("Step 4: Data Joining and Cleaning...")
    df_merged = pd.merge(df_qual, df_mapping, on='ticker', how='inner')
    df_final = pd.merge(df_merged, df_quant_filtered[['Financial Institution Name', 'Private_Credit_Proxy']],
                        on='Financial Institution Name', how='left')

    df_final['Loan_Volume_Billions'] = df_final['Private_Credit_Proxy'] / 1_000_000

    df_final = df_final.dropna(subset=['Loan_Volume_Billions'])
    df_final = df_final.sort_values(by='Loan_Volume_Billions', ascending=False).reset_index(drop=True)
    df_final['Color'] = df_final['sentiment'].apply(lambda x: color_map.get(x, '#7f7f7f'))

    MIN_VOLUME = 0.1  # $100M threshold
    df_show = df_final[df_final['Loan_Volume_Billions'] >= MIN_VOLUME].copy()
    df_rest = df_final[df_final['Loan_Volume_Billions'] < MIN_VOLUME]
    n_rest = len(df_rest)

    n_banks = len(df_show)
    print(f"Step 5: Generating Visualization ({n_banks} banks shown, {n_rest} grouped as ≈$0)...")
    sns.set_theme(style="whitegrid")

    tickers = list(df_show['ticker'])
    values = list(df_show['Loan_Volume_Billions'])
    ...
    for bar, color in zip(bars, colors):
        ...
        if height < MIN_VOLUME:
            continue
        plt.annotate(f'${height:,.1f}B',
                     xy=(bar.get_x() + bar.get_width() / 2, height),
                     xytext=(0, 5), textcoords="offset points",
                     ha='center', va='bottom', fontweight='bold',
                     fontsize=font_sz, color=color)

    plt.title('Private Credit Exposure vs. Management Sentiment', fontsize=16, fontweight='bold', pad=20)
    plt.xlabel('Bank Ticker', fontsize=12, fontweight='bold')
    plt.ylabel('Total Loan Volume ($ Billions)', fontsize=12, fontweight='bold')
...
if __name__ == "__main__":
    generate_hybrid_sentiment_chart()
